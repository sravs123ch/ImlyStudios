  ...
  useEffect(() => {
    if (isEditMode) {
      ...
      if (address) {
        ...
        const selectedCountry = countries.find(country => country.CountryID === address.CountryID) || null;
        if (selectedCountry) {
          setSelectedCountry(selectedCountry);
  
          // Fetch states based on selected country, if not already loaded
          if (!states.some(state => state.CountryID === selectedCountry.CountryID)) {
            fetchStatesByCountry(selectedCountry.CountryID);
          }
        }
  
        // Find and set selected state after country is set
        const selectedState = states.find(state => state.StateID === address.StateID) || null;
        if (selectedState) {
        ...
        const selectedCity = cities.find(city => city.CityID === address.CityID) || null;
